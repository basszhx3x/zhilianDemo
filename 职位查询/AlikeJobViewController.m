//
//  AlikeJobViewController.m
//  ZhiLianEmploy
//
//  Created by tony J on 13-3-6.
...
@interface AlikeJobViewController ()

@end
static int itag;
static int i;
@implementation AlikeJobViewController
...
-(void)dataJieShou
{
    ...
    RequestData *reqData = [RequestData requestDataWithURL:@"http://wapinterface.zhaopin.com/iphone/search/searchsamejob.aspx"
                                            withHttpMethod:@"GET"
                                                withParams:self.rootDic];
    [reqData setResultData:^(id aid, NSError *error) {
        NSLog(@"aiidd__________>%@",aid);
    
//    NSDictionary *resultDic = [NSMutableDictionary dictionary];
    if ([aid isKindOfClass:[NSMutableDictionary class]])
    {
        
        NSDictionary *dic = [aid valueForKey:@"root"];
//        resultDic = [dic valueForKey:@"result"];
//        NSLog(@"result-------->%@",resultDic);
        if ([[dic objectForKey:@"position"] isKindOfClass:[NSMutableDictionary class]]) 
        {
            NSDictionary *positonDic = [dic valueForKey:@"position"];
           // NSLog(@"positonDic------->%@",positonDic);
            NSMutableDictionary *dics = [positonDic valueForKey:@"city_id"];
            NSString *dicStr = [dics valueForKey:@"text"];            
            
            NSMutableDictionary *d = [positonDic valueForKey:@"job_number"];
            NSString *ds = nil;
            ...
            NSMutableDictionary *CompNamedic =[positonDic valueForKey:@"company_name"];
            ...
            NSMutableDictionary *CompNumberDic =  [positonDic valueForKey:@"company_number"];
            ...
            NSMutableDictionary *job_titledic = [positonDic valueForKey:@"job_title"];
            ...
            NSMutableDictionary *company_short_namedic = [positonDic valueForKey:@"company_short_name"];
        ...
        else
        {
            ...
            NSMutableArray *positionArr =  [dic valueForKey:@"position"];
            //NSLog(@"positionArr---------->%@",positionArr);
            
            for (NSMutableDictionary *dic in positionArr)
            {
                NSMutableDictionary *d = [dic valueForKey:@"job_number"];
                NSString *ds = nil;
                ds = [d valueForKey:@"text"];
                NSArray *dsarr = [ds componentsSeparatedByString:@" "];
                NSString *dsstrs = nil;
                ...
                NSMutableDictionary *dics =  [dic valueForKey:@"city_id"];
                NSString *dicStr = [dics valueForKey:@"text"];
                
                NSMutableDictionary *CompNamedic = [dic valueForKey:@"company_name"];
                NSString *CompNamedicS = [CompNamedic valueForKey:@"text"];
                NSArray *arrx = [CompNamedicS componentsSeparatedByString:@" "];
                NSString *CompNamedicStr = [arrx objectAtIndex:arrx.count-1];
                
                
                NSMutableDictionary *CompNumberDic = [dic valueForKey:@"company_number"];
                NSString *CompNumberDicS = [CompNumberDic valueForKey:@"text"];
                NSArray *arrx1 = [CompNumberDicS componentsSeparatedByString:@" "];
                NSString *CompNumberDicStr = [arrx1 objectAtIndex:arrx1.count-1];
                
                NSMutableDictionary *job_titledic = [dic valueForKey:@"job_title"];
                NSString *job_titledicS = [job_titledic valueForKey:@"text"];
                NSArray *arrx2 = [job_titledicS componentsSeparatedByString:@" "];
                
                NSString *job_titledicStr = [arrx2 objectAtIndex:arrx2.count-1];
                
                NSMutableDictionary *company_short_namedic =  [dic valueForKey:@"company_short_name"];
                NSString *company_short_namedicStr = [company_short_namedic valueForKey:@"text"];
