...
#import "AlikeJobViewController.h"
@interface JobMessageViewController ()

@end
static int i;
@implementation JobMessageViewController
@synthesize JobMesTextView = _JobMesTextView;
...
-(void)jobOperation:(UIButton *)sender
{
    switch (sender.tag)
    {
        ...
        case 5000:
        {
            uticketSingleTon *uticket = [uticketSingleTon share];
            NSMutableDictionary *dic = [NSMutableDictionary dictionary];
            
            [uticket.uticket substringToIndex:2];
            NSLog(@"%@",uticket.uticket);
            
            [dic setObject:[uticket.uticket removeSpaceAndEnter:uticket.uticket] forKey:@"uticket"];
            [dic setObject:[self.JobNumDic valueForKey:@"Job_number"] forKey:@"Job_number"];
            
            NSLog(@"%@",dic);
            ReportDataManage *reportManage = [[ReportDataManage alloc] init];
            [reportManage setfinishBlock:^(id result, id result1)
             {
                 strs = result;
                 NSLog(@"%@,%@",result,strs);
...
//                 [self shouCangAction];
            ...
            [reportManage readDataWithURL:@"http://wapinterface.zhaopin.com/iphone/job/collectposition.aspx" withHttpMethod:@"GET" Params:dic];
            [reportManage release];
            
        }
            break;
        case 6000:
            [self.JobNumDic setObject:@"1" forKey:@"Page"];
            [self.JobNumDic setObject:@"10" forKey:@"Pagesize"];
            NSLog(@"%@",self.JobNumDic);
            AlikeJobViewController *alike = [[AlikeJobViewController alloc] init];
            alike.rootDic = self.JobNumDic;
            [self.tabBarController.view viewWithTag:3000].hidden= NO;
            [self.cView removeFromSuperview];
            [self.navigationController pushViewController:alike animated:YES];
            [alike release];
            break;
        case 7000:
            NSLog(@"4");
...
//        NSLog(@"aid----------->%@",aid);
        ...
        NSDictionary *RootDic =[aid valueForKey:@"root"];
        NSDictionary *CurrentjobDic =[RootDic valueForKey:@"currentjob"];
        
//        公司介绍
        NSDictionary *companyDic  = [RootDic valueForKey:@"company"];

        
        NSDictionary *company_nameDic  = [companyDic valueForKey:@"company_name"];
        NSString *company_nameDicText  = [company_nameDic valueForKey:@"text"];
//        NSLog(@"%@",company_nameDicText);
        
//       公司性质 company_property
        NSDictionary *company_propertyDic = [companyDic valueForKey:@"company_property"];
        NSString *company_propertyDicText = [company_propertyDic valueForKey:@"text"];
//        NSLog(@"%@",company_propertyDicText);
        
//        公司规模  company_size
        
        NSDictionary *company_sizeDic = [companyDic valueForKey:@"company_size"];
        NSString *company_sizeDicText  = [company_sizeDic valueForKey:@"text"];
//        NSLog(@"%@",company_sizeDicText);
        
//      公司所属行业  industry
        
        NSDictionary *industryDic = [companyDic valueForKey:@"industry"];
        NSString *industryDicText  = [industryDic valueForKey:@"text"];
//        NSLog(@"%@",industryDicText);
        
//        地址  address
        
        NSDictionary *addressDic  = [companyDic valueForKey:@"address"];
        NSString *addressDicText = [addressDic valueForKey:@"text"];
//        NSLog(@"%@",addressDicText);
        
//        公司介绍  company_desc

        NSDictionary *company_descDic  = [companyDic valueForKey:@"company_desc"];
        NSString *company_descDicText  = [company_descDic valueForKey:@"text"];
...
//        职位介绍      
        ...
        NSDictionary *job_titleDic = [CurrentjobDic valueForKey:@"job_title"];
        NSString *Job_titleText = nil;
        Job_titleText = [job_titleDic valueForKey:@"text"];
        NSLog(@"%@",Job_titleText);
        NSDictionary *salary_rangeDic = [CurrentjobDic valueForKey:@"salary_range"];
        NSString *salary_rangeText = nil;
        salary_rangeText = [salary_rangeDic valueForKey:@"text"];
        NSLog(@"%@",salary_rangeText);
//        地点 job_city
        NSDictionary *job_cityDic = [CurrentjobDic valueForKey:@"job_city"];
        NSString *job_cityDicText =[job_cityDic valueForKey:@"text"];
        NSLog(@"%@",job_cityDicText);
//        经验  working_exp
        NSDictionary *working_expDic =  [CurrentjobDic valueForKey:@"working_exp"];
        NSString *working_expDicText = [working_expDic valueForKey:@"text"];
        NSLog(@"%@",working_expDicText);
//        人数  number
        NSDictionary *numberDic  = [CurrentjobDic valueForKey:@"number"];
        NSString *numberDicText = [numberDic valueForKey:@"text"];
        NSLog(@"%@",numberDicText);
        
//        职位描述  responsibility
        NSDictionary *responsibiltyDic = [CurrentjobDic valueForKey:@"responsibilty"];
