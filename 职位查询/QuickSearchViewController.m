...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        self.subTitleArray=[[[NSMutableArray alloc] initWithObjects:@"点击刷新获取当前位置",@"请选择职位名称",@"请选择行业",@"请选择工作地点",@"",@"不选择", nil] autorelease];
        ...
        self.indexpaths = [[[NSIndexPath alloc] init] autorelease];
        self.SalaryStr = [NSString string];

        self.ComSizeStr = [NSString string];

        self.changeCells = [[[ChangeCell alloc] init] autorelease];
...
-(void)dealloc
{   [_loationManage release];_loationManage.delegate=nil;
    [_SearchDic release];
...
//    NSLog(@"%@",[aInfo valueForKey:@"123"]);
    ...
    [self.SearchDic setObject:[sender.userInfo valueForKey:@"123"] forKey:@"city"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0)
    {
        ...
        if (indexPath.row == 4)
        {
            ...
            TextfieldCell *cell1 = [[[TextfieldCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:cells] autorelease];
  ...
  if (indexPath.section == 1) 
  {
      ...
      UITableViewCell *SectionCells = [[[UITableViewCell alloc] initWithStyle:(UITableViewCellStyleSubtitle) reuseIdentifier:cells] autorelease];
...
-(void)SalaryViewController:(SalaryViewController *)svc string:(NSString *)str
{
    ...
    {
    NSArray *arr1 = [NSArray arrayWithArray:[str componentsSeparatedByString:@"-"]];


    NSArray *arr  = [[arr1 objectAtIndex:1] componentsSeparatedByString:@"元"];
...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
   NSLog(@"%@",locations);
    CLLocation  *location =[locations lastObject];
      // MKReverseGeocoderInternal *geocoder = [MKReverseGeocoderInter]
    
    CLGeocoder *geoder = [[CLGeocoder alloc]init];
    [geoder reverseGeocodeLocation:location completionHandler:^(NSArray *placemarks, NSError *error) {
        
        NSLog(@"%@",[error description]);
        NSLog(@"%@",placemarks);
    
    }];
    self.loationManage.delegate = nil;
    [self.loationManage stopUpdatingLocation];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        if ([ CLLocationManager locationServicesEnabled]) {
            self.loationManage = [[[CLLocationManager alloc]init]autorelease];
            self.loationManage.delegate = self;
            self.loationManage.desiredAccuracy = kCLLocationAccuracyBest;
            self.loationManage.distanceFilter = 1000.0f;
            [self.loationManage startUpdatingLocation];
