...
#import "XMLReader.h"
@interface CompTypeViewController ()
...
- (id)initWithStyle:(UITableViewStyle)style
{
    ...
    if (self) {
        self.title = @"选择公司性质";
        self.comptypeArr = [NSMutableArray array];
//        [self.comptypeArr retain];
...
-(void)workExp
{
    ...
    NSMutableDictionary *comptypeDic = [basedataDic valueForKey:@"comptype"];
    NSMutableArray *ItemArr =  [comptypeDic valueForKey:@"item"];
    for (NSMutableDictionary *ItemDic in ItemArr)
    {
        ...
        [self.comptypeArr addObject:arr];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.textLabel.text = [[self.comptypeArr objectAtIndex:indexPath.row] objectAtIndex:1];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(CompTypeViewController:CompTypeStr:)])
    {
        [self.delegate CompTypeViewController:self CompTypeStr:[tableView cellForRowAtIndexPath:indexPath].textLabel.text ];
    }
