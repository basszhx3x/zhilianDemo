...
@implementation ChangeAreaViewController
...
-(NSMutableArray *)CityType
{
   ...
   // NSLog(@"cityDic--------------->%@",cityDic);
    ...
    NSMutableArray *itemArr = [firstLevelDic valueForKey:@"item"];
    ...
    for (NSMutableDictionary *EndDic in itemArr) 
    {
       ...
       NSMutableArray *arr1 = [NSMutableArray arrayWithObjects:Newstr,codeStr, nil];
            
        [self.ALLarr addObject:arr1];
        
        [self.arr addObject:Newstr];
    }
   // NSLog(@"arr==%@",self.ALLarr);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    // Navigation logic may go here. Create and push another view controller.
     ...
     PushCityViewController *detailViewController = [[PushCityViewController alloc] init];
     // ...
    detailViewController.arrfirstCode = [self.ALLarr objectAtIndex:indexPath.row];
